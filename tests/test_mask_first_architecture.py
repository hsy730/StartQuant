...
class TestTradableMaskConstruction:
    ...
    def test_limit_up_detection(self):
        ...
        # 验证检测结果
        assert result.iloc[20]["is_limit_up"], "第20天应该被检测为涨停"
        assert not result.iloc[20]["tradable_mask"], "涨停日应该是不可交易的"

        # 验证其他日期不受影响
        assert not result.iloc[19]["is_limit_up"], "第19天不应该是涨停"
        assert not result.iloc[21]["is_limit_up"], "第21天不应该是涨停"
    ...
    def test_limit_down_detection(self):
        ...
        assert result.iloc[30]["is_limit_down"], "第30天应该被检测为跌停"
        assert not result.iloc[30]["tradable_mask"], "跌停日应该是不可交易的"
    ...
    def test_suspended_detection(self):
        ...
        assert result.iloc[40]["is_suspended"], "第40天应该被检测为停牌"
        assert result.iloc[41]["is_suspended"], "第41天应该被检测为停牌"
        assert not result.iloc[40]["tradable_mask"], "停牌日应该是不可交易的"
    ...
    def test_price_limit_accuracy(self):
        """测试各市场板块的涨跌幅限制准确性"""
        # 主板股票（000001）- ±10%
        self.data_service._detect_price_limits(self.normal_data.copy(), "000001")
        assert (
            self.data_service._board_config[self.data_service._identify_market_board("000001")]["price_limit"] == 0.10
        )

        # 创业板股票（300001）- ±20%
        self.data_service._detect_price_limits(self.normal_data.copy(), "300001")
        assert (
            self.data_service._board_config[self.data_service._identify_market_board("300001")]["price_limit"] == 0.20
        )
...
class TestMaskedFactorOperators:
    ...
    def setup_method(self):
        ...
        # 正常价格序列
        self.prices = pd.Series(10 + np.cumsum(np.random.randn(self.n) * 0.5), index=self.dates, name="price")
        ...
        self.non_tradable_indices = np.random.choice(self.dates[10:-10], size=int(self.n * 0.1), replace=False)
    ...
    def test_ts_mean_with_mask(self):
        ...
        assert len(differences) > 0 or self.mask.all(), "Mask-First版本与传统版本应该有差异"

        # 验证可交易日的值不为NaN（大部分情况）
        tradable_count = (~result_masked.isna()).sum()
        total_count = len(result_masked)
        assert (
            tradable_count > total_count * 0.7
    ...
    def test_ts_corr_with_mask(self):
        ...
        # 使用mask版本
        result_masked = self.ts_corr_masked(self.prices, self.returns, n=20, mask=self.mask)

        # 验证结果范围在[-1, 1]之间
        valid_results = result_masked.dropna()
        if len(valid_results) > 0:
            assert (valid_results >= -1).all() and (valid_results <= 1).all(), "相关系数应该在[-1, 1]范围内"
    ...
    def test_backward_compatibility(self):
        ...
        # 调用无mask的masked版本
        self.ts_mean_masked(self.prices, n=20, mask=None)

        # 验证发出了警告
        log_output = log_stream.getvalue()
        assert "未提供mask" in log_output or "退化为" in log_output, "未提供mask时应该发出警告"
...
class TestBacktestIntegration:
    ...
    def setup_method(self):
        ...
        self.non_tradable_days = np.random.choice(self.dates[20:-20], size=int(self.n_days * 0.05), replace=False)
    ...
    def test_backtest_with_mask(self):
        """测试带mask的单因子回测"""
        result = self.bt.single_factor_backtest(self.df, factor_name="momentum_20", use_tradable_mask=True)
    ...
    def test_backtest_without_mask_fallback(self):
        ...
        result = self.bt.single_factor_backtest(
            df_no_mask, factor_name="momentum_20", use_tradable_mask=True  # 即使要求使用mask，但没有时也能运行
        )
...
class TestAnalysisEngineMaskIntegration:
    ...
    def setup_method(self):
        ...
        # 设置一些不可交易日
        non_tradable = np.random.choice(self.dates[20:-20], size=int(self.n_days * 0.08), replace=False)
    ...
    def test_ic_calculation_with_mask(self):
        """测试IC计算使用mask"""
        result = self.analysis._calculate_single_stock_ic(self.factor_data, ["momentum"], use_tradable_mask=True)
        ...
        if "momentum" in result["ic_stats"]:
            ...
            assert ic_stat["Mask-First"], "应该显示使用了Mask-First"
    ...
    def test_ic_without_mask_warning(self):
        ...
        result = self.analysis._calculate_single_stock_ic(factor_data_no_mask, ["momentum"], use_tradable_mask=True)
...
class TestPerformanceBenchmark:
    ...
    def test_large_dataset_performance(self):
        ...
        assert elapsed_time < 900.0, f"性能不达标: {elapsed_time:.2f}秒 > 900秒限制"
    ...
    def test_data_service_throughput(self):
        ...
        # 性能要求：单只股票应在0.5秒内完成
        assert elapsed_time < 0.5, f"性能不达标: {elapsed_time:.4f}秒 > 0.5秒限制"
