...
def test_independence_from_main_board():
    ...
    for industry_name in ["ChiNext_Tech", "ChiNext_Med", "ChiNew_Energy", "ChiNext_Consumer"]:
        industry_data = processed_chinext[processed_chinext["industry"] == industry_name]["factor_momentum"]
        ...
        if len(industry_data) > 0:
            ...
            print(
                f"  {status} {industry_name:20s}: 均值={mean_val:+7.4f}, "
...
def test_cross_sectional_isolation():
    ...
    print(f"  原始: 范围=[{orig_date1.min():.2f}, {orig_date1.max():.2f}], " f"标准差={orig_date1.std():.4f}")
    print(f"  处理后: 范围=[{proc_date1.min():.2f}, {proc_date1.max():.2f}], " f"标准差={proc_date1.std():.4f}")

    print(f"\n📅 日期2 ({date2.strftime('%Y-%m-%d')}):")
    print(f"  原始: 范围=[{orig_date2.min():.2f}, {orig_date2.max():.2f}], " f"标准差={orig_date2.std():.4f}")
    print(f"  处理后: 范围=[{proc_date2.min():.2f}, {proc_date2.max():.2f}], " f"标准差={proc_date2.std():.4f}")
...
def test_small_industry_handling():
    ...
    # 人为创建一个只有2只股票的"微小行业"
    tiny_industry_mask = (df["stock_code"] == f"{300001:06d}.SZ") | (df["stock_code"] == f"{300002:06d}.SZ")
    ...
    normal_data = processed_df[~tiny_industry_mask & (processed_df["industry"] == "ChiNext_Tech")]["factor_momentum"]
