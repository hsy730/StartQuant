...
def create_realistic_stock_data(
    ...
    if include_limit_up_down:
        ...
        limit_up_indices = np.random.choice(
            range(20, n_days - 20), size=n_limit_up, replace=False
        )
    ...
    if include_limit_up_down:
        ...
        limit_down_indices = np.random.choice(
            range(20, n_days - 20), size=n_limit_down, replace=False
        )
    ...
    suspended_indices = np.random.choice(
        range(30, n_days - 10), size=n_suspended, replace=False
    )
    
    # 确保不与涨跌停日重叠
    suspended_indices = [
        i for i in suspended_indices 
        if i not in limit_up_indices and i not in limit_down_indices
    ]
    
    for idx in suspended_indices[:n_suspended]:
        df.iloc[idx, df.columns.get_loc('volume')] = 0  # 成交量为0表示停牌
    
    logger.info(f"✅ 创建模拟数据 [{stock_code}] | "
                f"总天数: {n_days} | "
...
def test_genetic_mining_with_mask():
    ...
    if "tradable_mask" in processed_df.columns:
        ...
        print(f"   ✅ Mask-First已启用:")
        print(f"      总天数: {mask_stats['total']}")
        print(f"      可交易: {mask_stats['tradable']} ({mask_stats['ratio']*100:.1f}%)")
        print(f"      涨停: {mask_stats['limit_up']}, 跌停: {mask_stats['limit_down']}, 停牌: {mask_stats['suspended']}")
    else:
        ...
        if result.get("success", False):
            ...
            if len(best_factors) > 0:
                ...
                if len(progress_history) >= 2:
                    ...
                    improvement = (last_gen_best - first_gen_avg) / abs(first_gen_avg) * 100 if first_gen_avg != 0 else 0
...
def compare_with_without_mask():
    ...
    future_returns = pd.Series(
        0.001 * factor_values + np.random.randn(n_days) * 0.015,
        index=dates,
        name='future_return_1'
    )
    ...
    result_no_mask = analysis._calculate_single_stock_ic(
        factor_data_no_mask,
        ["test_factor"],
        use_tradable_mask=False  # ❌ 不使用mask
    )
    ...
    result_with_mask = analysis._calculate_single_stock_ic(
        factor_data_with_mask,
        ["test_factor"],
        use_tradable_mask=True  # ✅ 使用mask
    )
    ...
    if ic_diff > 0:
        print(f"   ✅ Mask-First使IC更保守（减少虚假信号）")
    elif ic_diff < -0.005:
        print(f"   ⚠️ IC反而升高，可能mask过于严格")
    else:
        print(f"   ➡️ IC变化不大，说明数据质量较好")
...
def performance_benchmark():
    ...
    from backend.services.factor_primitives import (
        ts_corr, ts_corr_masked,
        ts_mean, ts_mean_masked,
    )
    ...
    for n_samples in test_sizes:
        ...
        # 测试传统版本性能
        start = time.time()
        result_traditional = ts_corr(series_a, series_b, n=20)
        time_traditional = time.time() - start
        
        # 测试Mask-First版本性能
        start = time.time()
        result_masked = ts_corr_masked(series_a, series_b, n=20, mask=mask)
