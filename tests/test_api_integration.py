...
def test_ic_api():
    ...
    parts = []
    if "IC均值" in stat: parts.append(f"IC={stat['IC均值']:.4f}")
    if "IR" in stat: parts.append(f"IR={stat['IR']:.4f}")
    if "IC类型" in stat: parts.append(f"type={stat['IC类型']}")
    if "t统计量" in stat: parts.append(f"t={stat['t统计量']:.2f}")
    if "p值" in stat: parts.append(f"p={stat['p值']:.4f}")
