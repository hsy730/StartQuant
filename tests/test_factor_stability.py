...
class TestFactorStabilityService:
    ...
    def test_calculate_distribution_stability_with_sufficient_data(
        self, stability_service, sample_factor_data
    ):
        """
        测试分布稳定性计算 - 数据充足场景
        ...
        result = stability_service.calculate_distribution_stability(
            factor_series=sample_factor_data,
            window=252,
            method="ks"
        )
    ...
    def test_calculate_distribution_stability_insufficient_data(self, stability_service):
        ...
        with pytest.raises(ValueError) as exc_info:
            stability_service.calculate_distribution_stability(
                factor_series=short_data,
                window=252,
                method="ks"
            )
    ...
    def test_calculate_time_series_stationarity_stationary(
        self, stability_service, sample_ic_data
    ):
        """
        测试时间序列平稳性分析 - 平稳序列场景
        ...
        - 包含临界值信息
        """
        result = stability_service.calculate_time_series_stability(
            ic_series=sample_ic_data,
            maxlag=10
        )
    ...
    def test_calculate_time_series_stationarity_short_series(self, stability_service):
        ...
        with pytest.raises(ValueError) as exc_info:
            stability_service.calculate_time_series_stability(
                ic_series=short_ic,
                maxlag=10
            )
    ...
    def test_calculate_coefficient_of_variation_normal(
        self, stability_service, sample_ic_data
    ):
        """
        测试变异系数计算 - 正常数据场景
    ...
    def test_comprehensive_stability_test_success_scenario(
    ...
    def test_comprehensive_stability_test_nonexistent_factor(
        self, stability_service
    ):
        """
        测试综合稳定性检验 - 因子不存在场景
...
class TestFactorStabilityAPIIntegration:
    ...
    def test_stability_api_endpoint_structure(self):
        ...
        验证 API 路由定义是否正确
        """
        from backend.api.routers.analysis import router, StabilityRequest
