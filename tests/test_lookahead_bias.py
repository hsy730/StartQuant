...
class TestICDirectionCorrect:
    ...
    def test_ic_uses_next_day_return(self):
        ...
        factor = pd.Series(np.random.randn(20), index=dates, name="factor")
        ...
        if valid.sum() > 0:
            assert not np.allclose(next_return[valid].values, prev_return[valid].values), \
                "次日收益率和前一日收益率应不同"
...
class TestCumulativeReturnConversion:
    ...
    def test_correct_formula_restores_daily_returns(self):
        ...
        # 正确公式还原
        restored = [(cumulative[i+1] + 1) / (cumulative[i] + 1) - 1
                    for i in range(len(cumulative) - 1)]

        for i in range(len(daily_returns)):
            assert abs(restored[i] - daily_returns[i]) < 1e-10, \
    ...
    def test_wrong_diff_formula_does_not_restore(self):
        ...
        assert abs(wrong[1] - daily_returns[1]) > 1e-6, \
            "差分法在累计收益非零时应与原始日收益率不同"
