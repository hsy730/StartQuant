...
class TestFactorCorrelationService:
    ...
    def _make_sample_data():
        ...
        data = {
            ...
            "value": (
                np.random.normal(0, 1, n) * 0.8
                + pd.Series(np.random.normal(0, 1, n), index=index)
                .groupby("date")
...
class TestOptionalDependencies:
    ...
    def test_phik_fallback(self):
        ...
        df = pd.DataFrame(
            {"num_factor": [1, 2, 3, 4, 5], "cat_factor": ["A", "B", "A", "C", "B"], "another_num": [5, 4, 3, 2, 1]}
        )

        result = factor_correlation_service.analyze_mixed_type(
            df, ["num_factor", "cat_factor", "another_num"], categorical_cols=["cat_factor"]
        )
