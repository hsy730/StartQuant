...
- ✅ **5种挖掘算法** - 遗传规划(PySR/树预筛选/GFlowNet/深度隐式因子)
- ✅ **统一验收标准** - 各算法内部目标不同，但最终因子用同一套标准评分（[详见评估体系文档](docs/factor-mining-evaluation.md)）
...
FactorHub 选择 **VectorBT** 作为核心回测引擎，核心原因是**性能**——向量化回测比事件驱动引擎（如 Backtrader）快 **10-100 倍**，非常适合因子研究场景下的快速迭代与批量参数扫描。

| 对比维度 | VectorBT（向量化） | 事件驱动引擎（如 Backtrader） |
|---------|-------------------|------------------------------|
| 执行速度 | ⚡⚡⚡⚡⚡ 向量化运算 | 🐢 逐 bar 循环 |
| 批量回测 | 支持（可同时测试多参数组） | 通常单参数顺序执行 |
| 开发效率 | 快速迭代、代码简洁 | 需要更多样板代码 |
| 撮合精度 | 基于 bar 收盘/开盘价撮合 | 可模拟限价单、滑点等 |
| 订单类型 | 市价单为主 | 支持限价单、止损单等 |

**适用场景与限制：**

| 场景 | 适用性 | 说明 |
|------|--------|------|
| 因子有效性验证（日频/周频） | ✅ **非常适合** | 误差 < 0.5%，方向判断可靠 |
| 多因子组合优化 | ✅ **比较适合** | 关注排序和比率，绝对数值误差可接受 |
| 策略对比与初筛 | ✅ **非常适合** | 快速从大量策略中筛选候选者 |
| 分钟级高频策略 | ❌ **不适合** | 误差可能 > 2-5%，需改用事件驱动引擎 |
| 复杂订单逻辑（限价/止损/冰山） | ❌ **不支持** | VectorBT 无法模拟此类订单 |

**精度影响说明：**

- **收益指标**：年化收益率误差约 ±1-3%，对于判断"因子是否有 alpha"影响较小
- **风险指标**：最大回撤可能偏乐观 5-15%（假设完美成交），建议解读时预留安全边际
- **换手率**：可能低估 20-40%（未考虑市场冲击成本），高频调仓策略需特别注意
- **结论**：VectorBT 的精度完全满足**因子研究和策略初筛**的需求，但不适合**高频策略验证**或需要精确撮合模拟的场景
