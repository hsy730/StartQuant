...
class BacktestResultModel(Base):
    ...
    factor_combination: Mapped[str] = mapped_column(
        String(500), nullable=False, comment="因子组合（JSON字符串）"
    )
    start_date: Mapped[str] = mapped_column(
        String(20), nullable=False, comment="开始日期"
    )
    end_date: Mapped[str] = mapped_column(
        String(20), nullable=False, comment="结束日期"
    )
    initial_capital: Mapped[float] = mapped_column(
        Float, default=1000000, comment="初始资金"
    )
    final_capital: Mapped[float] = mapped_column(
        Float, nullable=True, comment="最终资金"
    )
    total_return: Mapped[float] = mapped_column(
        Float, nullable=True, comment="累计收益率"
    )
    annual_return: Mapped[float] = mapped_column(
        Float, nullable=True, comment="年化收益率"
    )
    volatility: Mapped[float] = mapped_column(
        Float, nullable=True, comment="年化波动率"
    )
    sharpe_ratio: Mapped[float] = mapped_column(
        Float, nullable=True, comment="夏普比率"
    )
    max_drawdown: Mapped[float] = mapped_column(
        Float, nullable=True, comment="最大回撤"
    )
    calmar_ratio: Mapped[float] = mapped_column(
        Float, nullable=True, comment="卡玛比率"
    )
    win_rate: Mapped[float] = mapped_column(Float, nullable=True, comment="胜率")
    sortino_ratio: Mapped[float] = mapped_column(
        Float, nullable=True, comment="索提诺比率"
    )
    equity_curve: Mapped[dict] = mapped_column(
        JSON, nullable=True, comment="净值曲线数据"
    )
    quantile_returns: Mapped[dict] = mapped_column(
        JSON, nullable=True, comment="各分层收益数据"
    )
    trades_count: Mapped[int] = mapped_column(Integer, default=0, comment="交易次数")
    benchmark_return: Mapped[float] = mapped_column(
        Float, nullable=True, comment="基准收益率"
    )
    excess_return: Mapped[float] = mapped_column(
        Float, nullable=True, comment="超额收益率"
    )
...
class TradeRecordModel(Base):
    ...
    trade_date: Mapped[str] = mapped_column(
        String(20), nullable=False, comment="交易日期"
    )
    action: Mapped[str] = mapped_column(
        String(10), nullable=False, comment="交易动作（buy/sell）"
    )
