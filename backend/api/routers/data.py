...
async def get_stock_data(code: str, start_date: str, end_date: str):
    ...
    """
    try:
        data = data_service.get_stock_data(stock_code=code, start_date=start_date, end_date=end_date)
            ...
            if df is not None and len(df) > 0:
                ...
                for col in df.columns:
                    ...
                    if "名称" in str(col) or "name" in str(col).lower():
                        name_col = col
                logger.info(f"index_stock_cons 成功获取 {symbol}, {len(df)} 条, 列: {df.columns.tolist()}")
        except Exception as e:
            ...
            try:
                ...
                if df is not None and len(df) > 0:
                    ...
                    for col in df.columns:
                        ...
                        if "名称" in str(col) or "name" in str(col).lower():
                            name_col = col
                    logger.info(f"index_stock_cons_csindex 成功获取 {symbol}, {len(df)} 条")
            except Exception as e:
            ...
            try:
                ...
                if df is not None and len(df) > 0:
                    ...
                    for col in df.columns:
                        ...
                        if "名称" in str(col) or "name" in str(col).lower():
                            name_col = col
                    logger.info(f"index_stock_cons_weight_csindex 成功获取 {symbol}, {len(df)} 条")
            except Exception as e:
                logger.warning(f"index_stock_cons_weight_csindex 获取 {symbol} 失败: {e}")

        if df is None or len(df) == 0:
            raise HTTPException(status_code=500, detail=f"所有数据源均无法获取指数 {symbol} 的成分股")
...
async def cleanup_cache():
    ...
    try:
        ...
        return {"success": True, "data": {"cleaned_count": cleaned}, "message": f"已清理 {cleaned} 个过期缓存"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def clear_cache():
    ...
    try:
        ...
        return {"success": True, "data": {"cleared_count": cleared}, "message": f"已清空 {cleared} 个缓存"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
