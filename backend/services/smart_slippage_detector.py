...
class SmartSlippageDetector:
    ...
    def recommend_slippage(
        ...
        (recommended, conservative, aggressive,
         confidence, reasoning, warnings, tips) = self._generate_recommendation(
    ...
    def _get_board_distribution_str(self, distribution: Dict[MarketBoard, float]) -> str:
        parts = [f"{self._get_board_name(k)}{v*100:.0f}%" for k, v in sorted(distribution.items(), key=lambda x: -x[1]) if v > 0.05]
