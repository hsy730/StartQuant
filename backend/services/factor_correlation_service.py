...
class FactorCorrelationService:
    ...
    def _rolling_stability(self, df, factor_cols, config) -> Dict:
        ...
        for i in range(0, len(dates) - window + 1, step):
            win_dates = dates[i : i + window]  # noqa: E203
                ...
                results.append(
                    {
                        ...
                        "regime": (
                            "high"
                            if np.mean(np.abs(corrs)) > 0.6
                            else ("low" if np.mean(np.abs(corrs)) < 0.25 else "normal")
                        ),
                    }
                )
    ...
    def _interpret_results(self, result: Dict) -> Dict:
        ...
        _n_low = len(interp["low_correlation_pairs"])  # noqa: F841
